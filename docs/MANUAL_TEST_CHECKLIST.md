...
> 이 문서의 기존 완료 표시는 아이언클래드 카드를 직접 교체하던 `v0.2.1` 기준 기록이다. 아래 바위클래드 최소 프로토타입 항목은 독립 캐릭터 전환 후 새로 확인해야 한다.

## 바위클래드 최소 프로토타입

- [ ] 캐릭터 선택 화면에 바위클래드가 별도 항목으로 표시
- [ ] 바위클래드 선택 화면에서 아이언클래드 배경·아이콘·전환 효과 재사용
- [ ] 전투·상점·휴식처에서 아이언클래드 외형·모션·음향 재사용
- [x] 카드 선택 풀에 바위 강타를 제외한 전용 카드 12장과 지정한 아이언클래드 카드 56장 등록
- [x] 바위 강타가 시작 덱에는 존재하지만 일반 카드 선택 풀에는 없음
- [x] 타락은 고대 카드로 유지되고 악마의 방패·탱커는 멀티플레이 전용으로 유지
- [x] 원본 아이언클래드 카드 풀에 바위클래드 카드가 포함되지 않음
- [x] 원본 바리케이드·악마의 형상·돌 갑옷·절대적인 힘·몸통 박치기가 바위 태그와 교체 효과를 갖지 않음
- [x] 시작 덱이 원본 아이언클래드의 타격 5장, 수비 4장과 바위 강타 1장으로 구성
- [x] 승천 4 이하에서는 등반자의 골칫거리가 없고, 승천 5 이상에서는 시작 덱에 1장 추가
- [x] 시작 유물이 불타는 혈액 1개
- [x] 전용 유물 풀에 바위 유물만 포함되고 아이언클래드 유물 풀에는 포함되지 않음
- [x] 신규 희귀 카드 4장의 비용·희귀도·강화 수치와 저장 직렬화 확인
- [ ] 바위함정이 버린 카드 더미의 모든 거대한 바위를 대상에게 사용하고, 강화 시 먼저 강화하는지 실제 게임 통합 셀프 테스트 확인
- [ ] 바위를 위한 모두가 피해를 준 뒤 버린 카드 더미의 모든 거대한 바위를 손으로 가져오는지 실제 게임 통합 셀프 테스트 확인
- [ ] 창의적인 인공바위가 턴 시작 시 시작 전용 바위 강타를 제외한 무작위 바위 관련 카드 1장을 생성하는지 실제 게임 통합 셀프 테스트 확인
- [ ] 바위성이 현재 및 이후 생성되는 거대한 바위에 재사용을 부여하고 다른 플레이어의 카드에는 적용하지 않는지 실제 게임 통합 셀프 테스트 확인
- [ ] 일반 전투 카드 보상에 바위클래드 카드 3장 표시
- [ ] 상점 카드가 바위클래드 카드 풀에서 생성
- [ ] 유물 보상에서 바위 유물이 등장
- [ ] 엘리트와 보스 처치 후 존재하지 않는 바위클래드 시대 조회로 중단되지 않음
- [ ] 저장 후 불러오기에서 캐릭터 ID, 덱, 불타는 혈액과 전용 카드가 유지
- [ ] 같은 모드 버전의 멀티플레이 로비에서 바위클래드 선택과 전투 동기화
